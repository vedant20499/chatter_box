...
// ---------- Speaking state ----------
let isSpeaking = false;          // true while the bot's voice is playing
...
function setSpeaking(active) {
  ...
  if (active) {
    ...
    // fallback: auto‑reset after 15 seconds if no end event fires
...
function playAudioBuffer(float32Array, sampleRate) {
  ...
  stopSpeaking();   // stop any previous sound
      ...
      if (SpeechRecognition) {
        ...
        this.recognition.onresult = (event) => {
          ...
          this.silenceTimeout = setTimeout(() => {
            // double‑check we aren't speaking anymore
            if (isSpeaking) return;
